var Slide = function(arg) {
	...
	this.$slides = [];															// 생성될 .yeonyi-slide(jQuery객체)를 저장할 배열
	...
	this.slideMargin = arg.slideMargin || 0					//step형에서 화면에 보여질 슬라이드 가로 마진
...
Slide.prototype.init = function(){
	...
	// stage에 슬라이드를 감싸는 yeonyi-wrapper를 생성
	this.$wrapper = $('<div class="yeonyi-wrapper yeonyi-'+this.direction+'"></div>').appendTo(this.$container);
	
	// yeonyi-wrapper에 yeonyi-slide들을 만드는 과정
	for(var i=0, html=''; i<this.slide.length; i++) {
		if(this.direction == 'step')
		html  = '<div class="yeonyi-slide" style="background-image:url(\''+this.slide[i]+'\');">';
		else
			html  = '<div class="yeonyi-slide">';
		html += '<img src="'+this.slide[i]+'" class="img">';
		if(this.title && this.title[i]) html += this.title[i];
		html += '</div>';
		
		// hori, vert 형은 yeonyi-wrapper에 실제 태그를 넣어놔야 되므로... 
		// 1. 실제태그도 붙이고 
	...
	// fade type은 0번 슬라이드를 yeonyi-wrapper에 붙인다.
	...
	if(this.btnUse) {
		this.$btnPrev = $('<div class="yeonyi-btn yeonyi-prev">〈</div>').appendTo(this.$container);
		this.$btnNext = $('<div class="yeonyi-btn yeonyi-next">〉</div>').appendTo(this.$container);
	...
	if(this.pagerUse) {
		...
		// yeonyi-pagers를 this.$container에 생성한다.
		this.$pagers = $('<div class="yeonyi-pagers"></div>').appendTo(this.$container);
		
		// 각각의 pager를 생성하고 클릭 이벤트를 적용한다.
		for(var i in this.slide) {
			$('<div class="yeonyi-pager"></div>').appendTo(this.$pagers).click(this.onPagerClick.bind(this));
		}

		// 0번 idx에 .active 클래스를 적용한다.
		this.$pagers.find(".yeonyi-pager").eq(0).addClass("active");
	}
...
html ='<img src= '
		...
		$(window).resize(function (){
			...
			if(w <= 991 && w > 767) this.stepCalc(cnt - 2 < 1 ? 1 : cnt - 1);
			if(w <= 767 && w > 575) this.stepCalc(cnt - 3 < 1 ? 1 : cnt - 1);
			if(w <= 575) this.stepCalc(cnt - 4 < 1 ? 1 : cnt - 1);
			
			this.$wrapper.css({
				"left":this.slideValue.leftValue + "%",
				"width":this.slideValue.parentWidth + "%",
			});
			this.$wrapper.find('yeonyi-slide').css({
				"flex":"clac(" + this.slideValue.chlidWidth+"% - " +this.slideMargin+ "px) 0 0",
...
// yeonyi-wrapper의 animation
Slide.prototype.ani = function() {
	if(this.pagerUse) {
		this.$pagers.find(".yeonyi-pager").eq(this.now).addClass("active").siblings().removeClass("active");
		if(this.direction === 'hori' || this.direction === 'vert') {
			if(this.now == this.last) this.$pagers.find(".yeonyi-pager").eq(0).addClass("active").siblings().removeClass("active");
		}
	}
	...
	if(this.direction === 'fade') {
		...
		Slide.prototype.stepCalc = function(cnt) {
			...
			this.slideValue.chlidWidth = (100 / (cnt + 2)).toFixed(4);
